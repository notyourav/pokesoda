		...
		case '%':
			...
			case 'p':
...

				{
                                    ...
                                    if ( type == 'l' )
                                    {
                                        if( (ch1 = _gethex( fin, ch1, &characters )) == EOF )
                                            return( count );	/* Conversion error */
                                        ptr_value = ch1;
                                        ptr_value <<= 8;   
                                        ch1 = fgetc( fin );
...
#if _MODEL == 's' || _MODEL == 'd'
                                    if ( type == 'l' )
                                        *(_far long *)(va_arg( ap, CHARP )) = ptr_value;
                                    else
                                        *(_near int *)(va_arg( ap, CHARP )) = ptr_value;
#else
                                    if ( type != 'h' )
                                        *(_far long *)(va_arg( ap, CHARP )) = ptr_value;
                                    else
