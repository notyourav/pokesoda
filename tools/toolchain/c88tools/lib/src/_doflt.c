...
#define IEEE_FORMAT(x)	(((_FDEF *)&value)->ieee_format.x)

static unsigned int
evalfloat( double value )
{
#ifndef _SINGLE_FP
    if(  IEEE_FORMAT(exponent) == EXP )
    {
	if(  IEEE_FORMAT(man_bits_15_0) == 0 )
	{
	    if(  IEEE_FORMAT(sign) == 0 )
	    {
		return 2;
	    }
	    else
	...
	if( ( IEEE_FORMAT(man_bits_15_0) == 0xFFFF) &&  (IEEE_FORMAT(sign) == 1) )
	{
	    return 1;
	}
    }
	...
	double * number, FILE *fout, unsigned int *characters )
{
	...
	unsigned int		IsNaNorInf = 0;
	
	/*
	 *
	 *	The include file _fpnan.h contains the code to test whether the
	 *	floating point operand is a NaN of infinity number. The conversion
	 ...
	 *	( they might raise exceptions ). Therefore the testroutine is target
	 *	dependent and located in a separate include file. The variable IsNaNorInf
	 *	is loaded with a numeric value which flags the operand value:
	 *		0  : Normal floating point number
	 *		1  : NaN
	 *		2  : Positive Infinity
	 *		3  : Negative Infinity
	 */
	...
	if( IsNaNorInf )
	{
	    switch( IsNaNorInf )
	    {
	        case 1 : fputs( "NaN", fout ); break;
		case 2 : fputs( "Infinity", fout ); break;
		case 3 : fputs( "-Infinity", fout );
