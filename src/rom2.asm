...
loc_0x01169B:
	...
	LD A, [IX+ENT_CUR_STATE_LO]
	LD B, [IX+ENT_CUR_STATE_HI]
	LD IY, BA
...
loc_0x0116DE:
	...
	LD A,[IX+ENT_CUR_STATE_LO] ; 11714
	LD B,[IX+ENT_CUR_STATE_HI] ; 11717
	LD IY,BA ; 1171a
...
loc_0x01172F:
	...
	LD IX, #@DOFF(objects) + 0C0h ; is this a separate list?
	LD XP, #@DPAG(objects)
...
loc_0x01173D:
	...
	LD A,[IX+ENT_CUR_STATE_LO] ; 11773
	LD B,[IX+ENT_CUR_STATE_HI] ; 11776
...
loc_0x0118D8:
	...
	LD A,[IX+ENT_CUR_STATE_LO] ; 118d8
	LD B,[IX+ENT_CUR_STATE_HI] ; 118db
	LD IY,BA ; 118de
...
loc_0x01196C:
	...
	LD A,[IX+ENT_CUR_STATE_LO] ; 1196c
	LD B,[IX+ENT_CUR_STATE_HI] ; 1196f
	LD IY,BA ; 11972
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
; Pet sprite state walking
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
global loc_0x01199D
loc_0x01199D:
	DB 0CFh, 0D8h, 0CEh ; 1199a
	ASCII "D" ; 119a0
	DB 04h, 0CEh ; 119a1
	ASCII "L" ; 119a3
...
global loc_0x0119C1
loc_0x0119C1:
	DB 0CFh, 0DBh, 0F8h, 0CFh, 0E6h ; 119be
...
loc_0x011B03:
	...
	LD A,[IY+04h] ; 11b17
	LD B,[IY+05h] ; 11b1a
	LD [IX+ENT_NEXT_STATE_LO],A ; 11b1d
	LD [IX+ENT_NEXT_STATE_HI],B ; 11b20
	...
	LD BA,#1A6Dh ; 11b6b
	LD [IX+ENT_CUR_STATE_LO],A ; 11b6e
	LD [IX+ENT_CUR_STATE_HI],B ; 11b71
...
loc_0x011BB3:
	...
	LD L,[IX+ENT_NEXT_STATE_LO] ; 11bcb
	LD H,[IX+ENT_NEXT_STATE_HI] ; 11bce
	LD IY,HL ; 11bd1
...
loc_0x011BE5:
	...
	LD BA,IY ; 11be5
	LD [IX+ENT_NEXT_STATE_LO],A ; 11be7
	LD [IX+ENT_NEXT_STATE_HI],B ; 11bea
	JRS loc_0x011C05
...
loc_0x011BEF:
	...
	LD BA,[IY] ; 11bf2
	LD [IX+ENT_CUR_STATE_LO],A ; 11bf4
	LD [IX+ENT_CUR_STATE_HI],B ; 11bf7

	ADD IY,#0002h ; 11bfa
	LD BA,IY ; 11bfd
	LD [IX+ENT_NEXT_STATE_LO],A ; 11bff
	LD [IX+ENT_NEXT_STATE_HI],B ; 11c02
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
; 0x011C7B - Updates UI objects
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
global object_ui_update_loop
object_ui_update_loop:
...
loc_0x011E29:
	...
	LD A,[IY+06h] ; 11e43
	LD B,[IY+07h] ; 11e46
	LD [IX+ENT_NEXT_STATE_LO],A ; 11e49
	LD [IX+ENT_NEXT_STATE_HI],B ; 11e4c
	...
	LD BA,#1A6Dh ; 11e93
	LD [IX+ENT_CUR_STATE_LO],A ; 11e96
	LD [IX+ENT_CUR_STATE_HI],B ; 11e99
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
; 0x011ECF - Executes object update routines
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
global object_update_loop
object_update_loop:

	; sprite array base address
	farload x, objects
...
loc_0x011EDA:
	...
	; load the state for this frame
	LD L, [IX+ENT_NEXT_STATE_LO]
	LD H, [IX+ENT_NEXT_STATE_HI]
	LD IY, HL

loc_0x011EFD:

	; load page
	LD A, [IX+ENT_STATE_PAGE]
	LD YP, A
...
loc_0x011F11:
	...
	LD [IX+ENT_NEXT_STATE_LO], A
	LD [IX+ENT_NEXT_STATE_HI], B
...
loc_0x011F1B:
	...
	LD BA,[IY]
	LD [IX+ENT_CUR_STATE_LO], A
	LD [IX+ENT_CUR_STATE_HI], B
...
pool_0121B3:
	...
	DB 0ADh, 0C1h, 99h, 0B9h, 21h
; ---------------------- ; 121c0
loc_0x0121C5:
	LD A,[keys_new_unpressed] ; 121c5
	BIT A,#KEY_A ; 121c9
	JRS NZ,loc_0x0121E9 ; 121cb
	BIT A,#KEY_B ; 121cd
	JRS Z,loc_0x012201 ; 121cf
	LD A,[1AB6h] ; 121d1
	AND A,A ; 121d5
	JRS Z,loc_0x0121DE ; 121d6
	LD A,#02h ; 121d8
	LD [14FAh],A ; 121da
loc_0x0121DE:
	LD HL,#86B5h ; 121de
...
loc_0x0121E9:
	PUSH IP ; 121e9
	PUSH IY ; 121ea
	LD YP,#@DPAG(pool_012261) ; 121eb
	LD IY,#@DOFF(pool_012261) ; 121ee
	LD A,[1B0Ah] ; 121f1
	CP A,#07h ; 121f5
	JRS C,loc_0x0121FB ; 121f7
	LD A,#07h ; 121f9
loc_0x0121FB:
	CARL loc_0x002586 ; 121fb
	POP IY ; 121fe
	POP IP ; 121ff
	RET
; ---------------------- ; 12200
loc_0x012201:
	LD A,[1B0Ah] ; 12201
	LD B,[unk1645] ; 12205
	BIT B,#KEY_LEFT ; 12209
	JRS NZ,loc_0x01221D ; 1220b
	BIT B,#KEY_RIGHT ; 1220d
	JRS NZ,loc_0x012220 ; 1220f
	BIT B,#KEY_UP ; 12211
	JRS NZ,loc_0x012223 ; 12213
	BIT B,#KEY_DOWN ; 12215
	JRS Z,loc_0x012238 ; 12217
	ADD A,#04h ; 12219
	JRS loc_0x012225
; ---------------------- ; 1221b
loc_0x01221D:
...
loc_0x012238:
	...
	POP IY ; 1224b
	POP IP ; 1224c
	CARL loc_0x007982 ; 1224d
	RET
; ---------------------- ; 12250
	DB 0Ch, 0Ch, 24h, 0Ch, 3Ch, 0Ch ; 12251
	ASCII "T" ; 12257
	DB 0Ch, 0Ch, 24h, 24h, 24h, 3Ch, 24h ; 12258
	ASCII "T$" ; 1225f
pool_012261:
	DW @COFF(loc_0x012271), @COFF(loc_0x0122A0)
    DW @COFF(loc_0x0122B4), @COFF(loc_0x012328)
    DW @COFF(loc_0x0122D4), @COFF(loc_0x012300)
    DW @COFF(loc_0x012314), @COFF(loc_0x0122E8)

; ---------------------- ; 1226a
loc_0x012271:
	LD A,[1AB6h] ; 12271
	AND A,A ; 12275
	JRS Z,loc_0x01227E ; 12276
	LD A,#06h ; 12278
	LD [14FAh],A ; 1227a
loc_0x01227E:
	LD A,#01h ; 1227e
...
; ---------------------- ; 124b9
pool_0124ba:
	DW pool_124c0, pool_124c8, pool_124d0

pool_124c0:
    DB 00h, 01h, 02h, 03h, 04h, 05h, 06h, 07h
pool_124c8:
    DB 00h, 04h, 03h, 05h, 02h, 01h, 06h, 07h
pool_124d0:
    DB 00h, 03h, 01h, 07h, 02h, 05h, 04h, 06h
...
loc_0x012787:
	...
	farcall loc_0x009B8D

	JRS NZ, loc_0x0127BE

	farcall loc_0x009B56
...
loc_0x012D6D:
	LD A,#01h ; 12d6d
	LD [1B14h],A ; 12d6f
	farcall loc_0x01898C ; 12d76
	LD BA,#0AC79h ; 12d79
	farcall loc_0x012E73 ; 12d7f
	RET
; ---------------------- ; 12d82
loc_0x012D83:
	LD A,[IX+02h] ; 12d83
	LD B,[IX+03h] ; 12d86
	farcall loc_0x012E73 ; 12d8c
	RET
; ---------------------- ; 12d8f
loc_0x012D90:
	LD BA,#0AC9Bh ; 12d90
	farcall loc_0x012E73 ; 12d96
	RET
; ---------------------- ; 12d99
loc_0x012D9A:
	LD A,[IX+14h] ; 12d9a
...
; ---------------------- ; 12ee1
loc_0x012EE2:
	farcall loc_0x00AA46 ; 12ee5
	JRS Z,loc_0x012EF6 ; 12ee8
	farcall loc_0x00C0E9 ; 12eed
...
loc_0x014C2C:
	...
	farcall loc_0x009ABE ; 14c37
	JRS NZ,loc_0x014C45 ; 14c3a
	CARL loc_0x012FF6 ; 14c3c
	farcall loc_0x00C0E9 ; 14c42
loc_0x014C45:
	RET
...
loc_0x014CCA:
	...
	BIT A,#KEY_LEFT ; 14cce
	JRS NZ,loc_0x014CE4 ; 14cd0

	BIT A,#KEY_RIGHT ; 14cd2
...
loc_0x015013:
	LD A,[1AE9h] ; 15013
	farcall loc_0x012476 ; 1501a
	CP A,#02h ; 1501d
	JRL Z,loc_0x015A33 ; 1501f
	CP A,#07h ; 15022
	JRL Z,loc_0x01636E ; 15024
	LD BA,#0003h ; 15027
	farcall loc_0x018FFE ; 1502d
	ADD A,A ; 15030
	LD B,#00h ; 15031
	PUSH IP ; 15033
	PUSH IX ; 15034
	farload x, pool_015042
	ADD IX,BA ; 1503b
	LD IY,[IX] ; 1503d
	POP IX ; 1503f
	POP IP ; 15040
...
loc_0x0156D5:
	...
	farcall loc_0x00BF32 ; 156dd
loc_0x0156E0:
	RET
; ---------------------- ; 156e0
loc_0x0156E1:
	LD A,[keys_new_unpressed] ; 156e1
	BIT A,#KEY_A ; 156e5
...
loc_0x0157CD:
	...
	farcall loc_0x009ABE ; 157d8
	JRS NZ,loc_0x0157E6 ; 157db
	CARL loc_0x012FFB ; 157dd
	farcall loc_0x00C0E9 ; 157e3
loc_0x0157E6:
	RET
; ---------------------- ; 157e6
loc_0x0157E7:
...
loc_0x0159F8:
	LD A,[keys_old] ; 159f8
	BIT A,#KEY_RIGHT ; 159fc
	JRS NZ,loc_0x015A0D ; 159fe
	BIT A,#KEY_LEFT ; 15a00
...
loc_0x015A33:
	...
	LD [IX+18h],A ; 15a35
	LD BA,#0007h ; 15a38
	farcall loc_0x018FFE ; 15a3e
	ADD A,A ; 15a41
	LD B,#00h ; 15a42
	PUSH IP ; 15a44
	PUSH IX ; 15a45
	farload x, pool_15a53
	ADD IX,BA ; 15a4c
	LD IY,[IX] ; 15a4e
	POP IX ; 15a50
	POP IP ; 15a51
...
loc_0x015B38:
	...
	farcall loc_0x009ABE ; 15b45
	POP A ; 15b48
	JRS NZ,loc_0x015B63 ; 15b4a
	PUSH A ; 15b4c
	CARL loc_0x013000 ; 15b4e
	farcall loc_0x00C0F7 ; 15b54
	LD A,#01h ; 15b57
	LD [1B3Dh],A ; 15b59
	POP A ; 15b5d
	ADD A,#24h ; 15b5f
...
loc_0x015CF5:
	LD A,[keys_old] ; 15cf5
	BIT A,#(KEY_UP | KEY_DOWN | KEY_LEFT) ; 15cf9
	JRS NZ,loc_0x015D06 ; 15cfb
	BIT A,#KEY_RIGHT ; 15cfd
...
loc_0x015D4B:
	LD A,[keys_old] ; 15d4b
	BIT A,#(KEY_UP | KEY_DOWN | KEY_RIGHT) ; 15d4f
	JRS NZ,loc_0x015D5C ; 15d51
	BIT A,#KEY_LEFT ; 15d53
...
loc_0x015DAC:
	LD A,[keys_old] ; 15dac
	BIT A,#KEY_RIGHT ; 15db0
	JRS NZ,loc_0x015DC2 ; 15db2
	BIT A,#KEY_LEFT ; 15db4
...
loc_0x016B47:
	...
	LD A,#1Dh ; 16b62
	LD [mn_pending_sfx],A ; 16b64
loc_0x016B68:
	LD [1B39h],IX ; 16b68
	LD HL,#1B43h ; 16b6b
	INC [HL] ; 16b6e
	farcall loc_0x009ABE ; 16b72
	JRS NZ,loc_0x016B80 ; 16b75
	CARL loc_0x012FF6 ; 16b77
	farcall loc_0x00C0E9 ; 16b7d
loc_0x016B80:
	PUSH IP ; 16b80
	PUSH IY ; 16b81
	LD A,[IX+18h] ; 16b82
...
loc_0x0171C4:
	LD A,[unk1645] ; 171c4
	BIT A,#KEY_LEFT ; 171c8
	JRS NZ,loc_0x0171DF ; 171ca
	BIT A,#KEY_RIGHT ; 171cc
...
loc_0x01760C:
	LD A,[unk1645] ; 1760c
	BIT A,#KEY_LEFT ; 17610
	JRS NZ,loc_0x017624 ; 17612
	BIT A,#KEY_RIGHT ; 17614
